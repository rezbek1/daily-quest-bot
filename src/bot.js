...
/quests — посмотреть невыполненные обязательства
/done [номер] — отметить, что пережил это
/profile — посмотреть, как ты развалился
/stats — статистика боли
/leaderboard — топ мучеников
/help — справка для отчаявшихся

Давай начнём. Создай свой первый квест:
/addtask Согласовать контракт

Или не начинай. Зависит от того, хочешь ли ты жить. 🖤
  `;
...
bot.command('addtask', async (ctx) => {
  ...
  if (!taskDescription) {
    await ctx.reply(
      '❌ Опишите задачу. Или продолжайте страдать в тишине.\n\nПример: /addtask Согласовать контракт'
    );
    return;
  }
  ...
  if (taskDescription.length > 200) {
    await ctx.reply(
      '❌ Настолько долгое описание можно было сразу выполнить.'
    );
    return;
  }

  // Показать статус генерации
  await ctx.reply('⏳ Генерирую сюжет... ChatGPT тоже выгорел, но пытается...');

  const quest = await createQuest(userId, taskDescription);

  if (!quest) {
    await ctx.reply('❌ Ошибка при создании квеста. Попробуй позже.');
    return;
  }

  const questMessage = `
✅ Квест создан! (Привет от бога алгоритмов)
...
💰 Награда: +${quest.xp} XP
⏱️ Сложность: ${'⭐'.repeat(Math.min(Math.floor(quest.xp / 20), 5))}

Переход к выполнению: /done ${quest.id.split('_').pop()}
  `;
...
bot.command('quests', async (ctx) => {
  ...
  message += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  message += `\n🎯 Выполни квест: /done [номер_квеста]\n`;
  message += `[Добавить еще боль] /addtask`;

  await ctx.reply(message);
});

/**
 * /done [квест_номер] - Завершить квест
 */
bot.command('done', async (ctx) => {
  ...
  if (!args) {
    ...
    quests.forEach((quest, index) => {
      message += `/done_${quest.id.split('_').pop()} - ${quest.title}\n`;
    });
  ...
  const questId = `quest_${userId}_${args}`;
